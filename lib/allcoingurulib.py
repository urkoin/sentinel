...
def is_valid_allcoinguru_address(address, network='mainnet'):
    ...
    allcoinguru_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Allcoinguru address
        return False

    if (address_version != allcoinguru_version):
        return False
...
# shims 'til we can fix the allcoingurud side
def SHIM_serialise_for_allcoingurud(sentinel_hex):
    from models import DASHD_GOVOBJ_TYPES
    # unpack
    obj = deserialise(sentinel_hex)

    # shim for allcoingurud
    govtype = obj[0]

    # add 'type' attribute
    obj[1]['type'] = DASHD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in allcoingurud
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # allcoingurud expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    allcoingurud_hex = serialise(obj)
    return allcoingurud_hex


# shims 'til we can fix the allcoingurud side
def SHIM_deserialise_from_allcoingurud(allcoingurud_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(allcoingurud_hex)

    # shim from allcoingurud
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('allcoinguru.conf')
