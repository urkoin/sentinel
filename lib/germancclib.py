...
def is_valid_germancc_address(address, network='mainnet'):
    ...
    germancc_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Germancc address
        return False

    if (address_version != germancc_version):
        return False
...
# shims 'til we can fix the germanccd side
def SHIM_serialise_for_germanccd(sentinel_hex):
    ...
    # superblock => "trigger" in germanccd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # germanccd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    germanccd_hex = serialise(obj)
    return germanccd_hex


# shims 'til we can fix the germanccd side
def SHIM_deserialise_from_germanccd(germanccd_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(germanccd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('germancc.conf')
