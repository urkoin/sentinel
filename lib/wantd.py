"""
allcoingurud JSONRPC interface
"""
import sys
import os
...
class AllcoinguruDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some allcoingurud methods
    ...
    @classmethod
    def from_allcoinguru_conf(self, allcoinguru_dot_conf):
        from allcoinguru_config import AllcoinguruConfig
        config_text = AllcoinguruConfig.slurp_config_file(allcoinguru_dot_conf)
        creds = AllcoinguruConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = allcoingurulib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = allcoingurulib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        DO NOT CALL DIRECTLY if you don't allcoinguru a "Oh Noes." exception.
        """
        current_block_height = self.rpc_command('getblockcount')
        ...
        future_seconds = allcoingurulib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_allcoingurud_proto_version_with_sentinel_ping)
